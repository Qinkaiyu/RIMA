...
class QuadraticProblem:
  ...
  def __init__(
      ...
      labels_a: Optional[jnp.ndarray] = None,
      labels_b: Optional[jnp.ndarray] = None,
      n_labels: Optional[jnp.ndarray] = None,
      block_diag_mat: Optional[jnp.ndarray] = None,
  ):
  ...
  def marginal_dependent_cost_labeled(
      self,
      marginal_1: jnp.ndarray,
      marginal_2: jnp.ndarray,
      labels_1: jnp.ndarray,
      labels_2: jnp.ndarray,
    ...
    if self._loss_name == "sqeucl":  # quadratic apply, efficient for LR
      for l in jnp.unique(labels_1, size = self.n_labels):
        tmp1_l = geom_xx.apply_square_cost(marginal_1, axis=1, sub_idx = labels_1 == l)
        tmp2_l = geom_yy.apply_square_cost(marginal_2, axis=1, sub_idx = labels_2 == l)
        if tmp1 is None: tmp1 = tmp1_l
        else: tmp1 += tmp1_l
        if tmp2 is None: tmp2 = tmp2_l
        else: tmp2 += tmp2_l
    else:
      f1, f2 = self.linear_loss
      for l in jnp.unique(labels_1, size = self.n_labels):
        tmp1_l = apply_cost(geom_xx, marginal_1, axis=1, fn=f1, sub_idx = labels_1 == l)
        tmp2_l = apply_cost(geom_yy, marginal_2, axis=1, fn=f2, sub_idx = labels_2 == l)
        if tmp1 is None: tmp1 = tmp1_l
        else: tmp1 += tmp1_l
        if tmp2 is None: tmp2 = tmp2_l
        else: tmp2 += tmp2_l
  ...
  def update_linearization(
    ...
    if self.labels_a is None:
      marginal_cost = self.marginal_dependent_cost(marginal_1, marginal_2)
    else:
      marginal_cost = self.marginal_dependent_cost_labeled(marginal_1, marginal_2, 
    ...
    if self.labels_a is not None:
      print("Label considered for Sinkhorn run")
      cost_matrix = cost_matrix + (1 / self.block_diag_mat -1)
    geom = geometry.Geometry(
    ...
    return linear_problem.LinearProblem(
        ...
        labels_a = self.labels_a, labels_b = self.labels_b
    )
