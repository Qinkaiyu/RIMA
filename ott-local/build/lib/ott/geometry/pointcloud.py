...
class PointCloud(geometry.Geometry):
  ...
  def apply_cost(
    ...
    return self._apply_cost(arr, axis, fn=fn, sub_idx=None)

  def _apply_cost(
      self, arr: jnp.ndarray, axis: int = 0, fn=None, sub_idx=None
  ) -> jnp.ndarray:
    """See :meth:`apply_cost`."""
    if not self.is_online or sub_idx is not None:
      return super().apply_cost(arr, axis, fn, sub_idx=sub_idx)
