...
class ICNN(neuraldual.BaseW2NeuralDual):
    ...
    init_fn: Callable[[jax.Array, Tuple[int, ...], Any], jnp.ndarray] = (
        DEFAULT_KERNEL_INIT
    )
...
class MLP(neuraldual.BaseW2NeuralDual):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:  # noqa: D102
        ...
        if self.is_potential:
            ...
            z += quad_term
        elif self.dim_out is None:
            Wx = nn.Dense(n_input, use_bias=True)
            z = x + Wx(z)
        else:
            Wx = nn.Dense(self.dim_out, use_bias=True)
            z = Wx(z)
...
class MetaInitializer(lin_init.DefaultInitializer):
    ...
    **same** geometry ``geom`` is used throughout, both for training and
    ...
    def __init__(
        ...
        opt: Optional[optax.GradientTransformation] = optax.adam(
            learning_rate=1e-3
        ),  # noqa: B008
        ...
        if state is None:
            ...
            placeholder = jnp.concatenate(
                [jnp.zeros(na), jnp.zeros(nb)], axis=-1
            )
    ...
    def init_dual_a(  # noqa: D102
        ...
        return (
            init_f if lse_mode else ot_prob.geom.scaling_from_potential(init_f)
        )
    ...
    def _compute_f(
        ...
        """
        return self.meta_model.apply(
            {"params": params}, jnp.concatenate([a, b], axis=-1)
        )
