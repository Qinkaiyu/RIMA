...
class Geometry:
  ...
  def apply_square_cost(self, arr: jnp.ndarray, axis: int = 0, sub_idx = None) -> jnp.ndarray:
    ...
    """
    return self.apply_cost(arr, axis=axis, fn=lambda x: x ** 2, sub_idx = sub_idx)
  ...
  def apply_cost(
    ...
    app = functools.partial(self._apply_cost_to_vec, axis=axis, fn=fn, sub_idx=sub_idx,**kwargs)
  ...
  def _apply_cost_to_vec(
    ...
    matrix = self.cost_matrix[sub_idx, :].T if axis == 0 else self.cost_matrix[:, sub_idx]
    matrix = fn(matrix) if fn is not None else matrix
    return jnp.dot(matrix, vec[sub_idx])
